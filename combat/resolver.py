...
def resolve_attack(attacker, defender, blocked, current_combo):
    # Уворот полностью отменяет атаку и сбрасывает серию.
    ...
    # Урон завязан на силе: каждая сила даёт 3 урона, выносливость снижает его на 0.5.
    dice = (0, 0)
    ...
    if blocked:
        if critical:
            ...
            return {
                "damage": damage, "dice": dice, "critical": True,
                "critical_dice": critical_dice, "critical_multiplier": critical_multiplier,
                "blocked": True, "dodged": False, "combo_level": combo_level,
            }
